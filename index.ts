import { IHostContext, IUserContext, HostType, IExtensionInitOptions, ContextIdentifier, ITeamContext, IExtensionContext, IPageContext } from "azure-devops-extension-sdk";
...
export const init = (options?: IExtensionInitOptions): Promise<void> => {
    return new Promise((resolve) => options?.loaded ? setTimeout(() => resolve(), 1000) : resolve());
}
...
export const getConfiguration = (params?: { [key: string]: any }) => {
    return params ? params : { id: faker.string.uuid(), version: faker.system.semver(), name: faker.lorem.slug(), publisherId: faker.lorem.slug(), commitSha: faker.git.commitSha() };
}

/**
 * Mocking SDK.getContributionId returns some Id
 */
export function getContributionId() { return faker.lorem.slug() }
...
export const getAppToken = () => new Promise((resolve) => resolve(Buffer.from(faker.string.uuid()).toString('base64')));

export const getAccessToken = () => new Promise((resolve) => resolve(Buffer.from(faker.string.uuid()).toString('base64')));
